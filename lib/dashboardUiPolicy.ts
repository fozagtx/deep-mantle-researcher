 ...
 * - References Pereira/Canelo, Colapinto, Álvarez: solo si el usuario no tiene al menos
 *   un VS propio en `open` o `accepted` con id “real” (no invalid off-chain ids).
 *
 * Dashboard UI — Fase 1
 *
 ...
 * - Carga inicial: esqueleto en la lista hasta tener snapshot.
...
/**
 * Muestra filas auxiliares solo cuando no hay exposición activa on-chain.
 */
export function shouldShowDashboardStakeHoldingsReferences(duels: VSData[]): boolean {
  const hasRealActive = duels.some(
    (d) =>
      (d.state === "open" || d.state === "accepted") &&
      !isInvalidOnChainVsId(d.id)
  );
