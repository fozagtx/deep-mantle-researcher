  ...
  const loadExploreData = useCallback(
    async ({
      ...
      if (!opportunitiesEnabled) {
        ...
        opportunitiesPromise = fetch("/api/challenge-opportunities?limit=8", {
          cache: "no-store",
        })
          .then(async (response) => {
    ...
    activeView === "open"
      ...
      : activeView === "closed"
        ? {
            ...
            hint: "Markets Branium has settled or refunded. Each card opens to the on-chain settlement receipt.",
          }
        : {
    ...
    if (filteredClosedChallenges.length === 0) {
      return (
        <ExploreArenaEmptyState
          ...
          title="No closed claims here yet"
          description="Settled claims appear once the deadline passes and Branium posts a verdict on chain."
