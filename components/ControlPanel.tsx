    ...
    >
      {label && (
        <div className="px-4 py-2 border-b border-black/[0.06]">
          <span className="font-mono text-[10px] font-bold uppercase tracking-[0.18em] text-pv-muted/90">
            {label}
          </span>
        </div>
      )}
...
export function SegmentedSwitch({
  ...
  return (
    <div
      className={`inline-flex rounded-lg bg-white border border-black/[0.08] p-0.5 ${className}`}
    >
      {options.map((opt) => (
        <button
          key={opt.value}
...
export function DataBadge({
  ...
  return (
    <div className="flex flex-col gap-0.5">
      <span className="font-mono text-[9px] uppercase tracking-[0.15em] text-pv-muted/90">
        {label}
      </span>
