...
export default function Artifact({
  ...
  return (
    <motion.div
      className={`relative border border-black/[0.12] rounded-lg bg-white overflow-hidden ${
    ...
    >
      ...
      {(serialNumber || stamp) && (
        <div className="flex items-center justify-between px-4 py-2 border-b border-black/[0.06] bg-black/[0.02]">
          {serialNumber && (
            <span className="font-mono text-[10px] tracking-[0.15em] text-pv-muted/90 uppercase">
              {serialNumber}
            </span>
          )}
