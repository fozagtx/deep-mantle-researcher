  ...
  return (
    <div className={`flex flex-col ${className}`}>
      {label && labelPosition === "above" && (
        <span
          className={`font-mono text-[10px] font-bold uppercase tracking-[0.15em] text-pv-muted/90 mb-1 ${labelClassName}`}
        >
          {label}
        </span>
      )}
      ...
      >
        {prefix && <span className="text-pv-muted/90">{prefix}</span>}
        {displayValue}
        {suffix && (
          <span className="text-[0.6em] ml-1 text-pv-muted/90 font-mono font-normal">
            {suffix}
          </span>
        )}
      </motion.div>
      ...
      {label && labelPosition === "below" && (
        <span
          className={`font-mono text-[10px] font-bold uppercase tracking-[0.15em] text-pv-muted/90 mt-1 ${labelClassName}`}
        >
          {label}
        </span>
      )}
