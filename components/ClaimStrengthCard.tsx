          ...
          {result.signals.map((signal) => (
            <li
              ...
              className={`flex items-start gap-2 text-xs leading-relaxed ${
                signal.passed ? "text-pv-text" : "text-pv-muted"
              }`}
            >
              {signal.passed ? (
                <CheckCircle2
                ...
                <CircleDashed
                  size={14}
                  className="mt-0.5 shrink-0 text-pv-muted/90"
                  aria-hidden
                />
              )}
          ...
          <div className="rounded-xl border border-black/[0.12] bg-black/[0.03] px-3 py-2">
            <div className="flex items-stretch gap-3 text-xs leading-relaxed">
                  ...
                  moderationCodeChips.length > 0 ? (
                    <div className="mt-2 flex flex-wrap gap-1.5">
                      {moderationCodeChips.map((chip) => (
                        <span
                          key={chip.code}
                          className="rounded-full border border-black/[0.12] bg-black/[0.04] px-2 py-0.5 text-[10px] font-medium text-pv-text"
                        >
                          {chip.label}
                        </span>
                      ))}
              ...
              {moderation.status !== "checking" && moderationCheckedLabel ? (
                <div className="flex shrink-0 items-center self-stretch text-right text-[10px] font-medium text-pv-muted/90">
