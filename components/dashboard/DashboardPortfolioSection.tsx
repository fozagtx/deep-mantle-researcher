...
} from "@/lib/dashboardStakeHoldingsReference";
import {
...
import { isInvalidOnChainVsId } from "@/lib/vs-ids";
import {
...
type StakeHoldingOpenKey = `reference:${DashboardStakeHoldingId}` | `vs:${number}`;
...
function FilteredExposureSummaryStrip({
  ...
  const t = useTranslations("dashboard");
  const sep = (
    <span className="text-pv-muted/90" aria-hidden>
      {" "}
      ·{" "}
    </span>
  );
...
function StakeHoldingsColumn({
  ...
  showStakeHoldingsReferences: boolean;
  exposureFilterKey: string;
  onResetFilters: () => void;
  totalDuelsCount: number;
  /** Primera carga sin snapshot aún: esqueleto y sin references ni “vacío”. */
  ...
  useEffect(() => {
    if (openKey?.startsWith("reference:")) {
      const id = openKey.slice("reference:".length) as DashboardStakeHoldingId;
  ...
  const referenceOnlySearchEmpty =
    !isInitialExposureLoad &&
    referenceSearchEmpty &&
    showStakeHoldingsReferences &&
    filteredVsList.length === 0;

  const showReferencesBlock =
    !isInitialExposureLoad && showStakeHoldingsReferences && !referenceSearchEmpty;
  ...
  return (
    ...
    >
      ...
      >
        ...
        {!isInitialExposureLoad && referenceOnlySearchEmpty ? (
          <p
        ...
        {!isInitialExposureLoad && referenceSearchEmpty && !referenceOnlySearchEmpty ? (
          <p
            className={`${DASHBOARD_SURFACE_DASHED} px-4 py-6 text-center font-mono text-xs text-pv-muted sm:text-sm`}
            role="status"
          >
        ...
        {showReferencesBlock
          ? visibleHoldingIds.map((id) => (
              <StakeHoldingRow
                key={id}
                id={id}
                isOpen={openKey === `reference:${id}`}
                onToggle={() =>
                  setOpenKey(openKey === `reference:${id}` ? null : `reference:${id}`)
                }
              />
            ))
          : null}
...
export default function DashboardPortfolioSection({
  ...
  /** Misma cadena que el input de búsqueda del filtro VS (filtra filas de tenencias reference). */
  stakeHoldingsSearchQuery?: string;
  ...
  return (
    <div className="mb-10 grid grid-cols-1 gap-8 lg:grid-cols-12 lg:gap-10 xl:gap-12">
      <StakeHoldingsColumn
        ...
        showStakeHoldingsReferences={showStakeHoldingsReferences}
